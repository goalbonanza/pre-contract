...
contract CrowdsaleToken is StandardToken, Ownable {
  ...
  string public standard = "GoalBonanza Token v1.0";
  string public name = "GoalBonanza Goal";
  ...
  function createTokens(address _contributor) payable {
    ...
    uint tokenAmount = calculateEthToGoal(contributionAmount);      // Calculate how much tokens must contributor get
    if (tokenAmount > 0) {
  ...
  function calculateEthToGoal(uint _eth) constant returns(uint) {
    ...
    return tokens + getStageBonus(percentage, tokens);
  }
